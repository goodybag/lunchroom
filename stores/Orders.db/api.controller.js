...
console.log("ORDER RECEIVED", data.attributes);
	...
	// TODO: Repeat on non-fatal error.
				...
				function storeOrder (data, paymentCharge, extraInfo) {
					...
					return Q.fcall(function () {
						...
						*/
						...
						return ENDPOINTS.Store.bookshelf.create.call(store, model, data).then(function (resp) {
							...
							// Now that the order is stored we insert the order item
							// records to track fulfillment and delivery.
							Object.keys(extraInfo.itemsByDay).forEach(function (day_id) {
								extraInfo.itemsByDay[day_id].items.forEach(function (item) {
									records.push({
										"order_id": resp.attributes.id,
										...
										"day_id": day_id,
										"placed_day_id": data.attributes.day_id
									});
								});
							});
...
console.log("sendNotifications extraInfo", JSON.stringify(extraInfo, null, 4));
				...
				return groupItemsByDayAndResolve(data).then(function (extraInfo) {
					...
					return chargeCard(data).then(function (paymentCharge) {
						...
						return storeOrder(data, paymentCharge, extraInfo).then(function (resp) {
...
var upstreamUpdate = store.update;
store.update = function (model, params) {
	...
	return new Promise(function (resolve, reject) {
		return qknex('order-items', function (table) {
			return table.where('order_id', model.attributes.id).delete();
		}).then(resolve, reject);
	}).then(function () {

		return upstreamUpdate(model, params);
	});
}
