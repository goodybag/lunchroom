...
exports.up = function (knex, Promise) {
	...
	return knex.schema
		.createTable(TABLE_NAME, function (t) {
			...
			t.text('day_id').notNullable();
			t.json('form').notNullable();
			t.json('items').notNullable();
			t.json('event').notNullable();
			t.text('vendor_ids').notNullable();
			t.text('orderFrom').notNullable();
			t.text('deliveryStartTime').notNullable();
			t.text('deliveryEndTime').notNullable();
